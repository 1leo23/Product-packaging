...
SLICE_COLLECTION = os.getenv("SLICE_COLLECTION")
SECRET_KEY = os.getenv("SECRET_KEY")
STORAGE_ROOT = os.getenv("STORAGE_ROOT","data")

client = MongoClient(MONGO_URI)
db = client[DATABASE_NAME]
slice_collection = db[SLICE_COLLECTION]
...
# ========== 3. 工具函式 ========== #
def slice_2d(slice_2d: np.ndarray) -> str:
...
@app.post("/upload/nii_slice")
async def nii_slice(file: UploadFile = File(...)):
    """
    ...
    # 轉成Base64
    sag_b64_list = [slice_2d(slc) for slc in sag_slices]
    cor_b64_list = [slice_2d(slc) for slc in cor_slices]
    axi_b64_list = [slice_2d(slc) for slc in axi_slices]
    ...
    result = slice_collection.insert_one(doc)
...
def get_3plane_slices(doc_id: str):
    doc = slice_collection.find_one({"_id": ObjectId(doc_id)})
    if not doc:
